...
export function ExportMenu({ ir, flowId, fileName }: Props) {
  ...
  const renderGif = async (): Promise<Blob> => {
    ...
    const probe = await toSvg(ir, { flowId, light, timeMs: 0 });
    ...
    for (let i = 0; i < frameCount; i++) {
      ...
      const svg = await toSvg(ir, { flowId, light, timeMs: (i / frameCount) * loopMs });
    ...
    setProgress(100);
    return bytesToBlob(encodeGif(frames, palette, { ...size, delayMs: 1000 / fps }), 'image/gif');
  };
    ...
    try {
      ...
      switch (format) {
        ...
        case 'archimate':
          ...
          break;
        case 'pdf': {
          const canvas = await draw(await toSvg(ir, { ...svgOptions, light: true }), scale, '#ffffff');
